  ...
  results: {
                  ...
                  child: {
                    ...
                    diemCC: {
                      ...
                      child: {
                        ...
                        tip: {
                          tag: "tip",
                          title: "Điểm Chuyên Cần (weighted 10%)",
                        },
                      },
                    },
                    ...
                    diemDK: {
                      ...
                      child: {
                        ...
                        tip: {
                          tag: "tip",
                          title: "Điểm Điều Kiện (weighted 20%)",
                        },
                      },
                    },
      ...
      core.account.onLogin(async () => {
        ...
        if (resultCache) {
          ...
          if (resultCache.email === core.account.email) {
            this.log(
              "INFO",
              "Account data has been cached, we don't need to fetch it again."
            );
        ...
        if (needUpdate || this.screen.showing) await this.load();
      });

      api.onResponse("results", (response) => {
        if (!this.loaded) this.screen.overlay({ show: false });
      ...
      this.screen.overlay({
        ...
        buttons: {
          login: {
            text: "ĐĂNG NHẬP",
            icon: "signin",
            onClick: () => (core.account.clickable.active = true),
          },
        },
      });
    ...
    async scan() {
      if (!this.currentData)
        throw {
          code: 22,
          description: `core.screen.results.scan(): no data available`,
        };

      /** @type {ResultGroupStore[]} */
      let groupingData = localStorage.getItem("results.grouping");
      groupingData = groupingData ? JSON.parse(groupingData) : [];
      ...
      popup
        .show({
          ...
          description:
            "Trình quét hiện đang quét lịch học của bạn để xác định thời gian học cho các môn. Quá trình này có thể mất 1-2 phút.",
        ...
        .then((action) => {
          if (action === "close") cancelled = true;
        });
      ...
      for (let result of this.currentData.info.results) {
        ...
        if (match && !years.includes(match[1])) years.push(match[1]);
      }

      this.log(
        "DEBG",
        "core.screen.results.scan(): list of years needed to scan:",
        years
      );
      ...
      for (let year of years) {
        for (let semester of [1, 2, 3]) {
          this.log(
            "INFO",
            `core.screen.results.scan(): scanning subjects of year ${year} semester ${semester}`
          );
          ...
          for (let date of dates) {
            ...
            this.log(
              "DEBG",
              `core.screen.results.scan(): fetching ${dateString}`
            );
    ...
    render(data, force = false) {
      ...
      if (newData || force) {
        ...
        for (let item of data.info.results) this.addListItem(item);
      }
    },
    ...
    addListItem({
      ...
      let row = makeTree("tr", "item", {
        ...
        gradePoint: {
          tag: "td",
          class: ["right", "bold"],
          text: grade ? grade.point.toFixed(2) : "",
        },
